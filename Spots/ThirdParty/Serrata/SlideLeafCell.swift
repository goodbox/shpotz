...
public final class SlideLeafCell: UICollectionViewCell {
    ...
    public func configure(slideLeaf: SlideLeaf) {
        if let image = slideLeaf.image {
            ...
            imageView.af_setImage(withURL: URL(string: url)!, placeholderImage: nil, filter: nil, progress: nil, progressQueue: .main, imageTransition: .crossDissolve(0.2), runImageTransitionIfCached: false) { (data) in
                if data.value == nil { return }
                
                self.activityIndicatorView.isHidden = true
                self.activityIndicatorView.stopAnimating()
                self.setImage(data.value!)
            }
            /*
